...
def validate_manifest(manifest):
    ...
    require_keys(
        outputs,
        set(EXPECTED_GENERATED_OUTPUTS),
        set(),
        context + ".generated_outputs",
    )
    if outputs != EXPECTED_GENERATED_OUTPUTS:
        raise RegistryError(
            "{}.generated_outputs must equal the protected output mapping: {}".format(
...
def render_llms(manifest, index):
    papers = index["papers"]
    claim_indexed_count = sum(
        paper["entry_kind"] == "claim-indexed" for paper in papers
    )
    identity_labels = sorted(
        paper["inventory_label"]
        for paper in papers
        if paper["entry_kind"] == "identity-only"
    )
    paper_count = index["counts"]["papers"]

    lines = [
        "# Generation in the Limit Lean Library",
        "",
        "This file is generated by `scripts/build_registry.py`. Do not edit it by hand.",
        "",
        "Machine-readable LLM card stream:",
        manifest["generated_outputs"]["cards"],
        "",
        "Machine-readable lookup and facet index:",
        manifest["generated_outputs"]["index"],
        "",
        "Automatically exported Lean declaration card stream:",
        "registry/generated/declarations.jsonl",
        "",
        "Lean declaration lookup and facet index:",
        "registry/generated/declaration-index.json",
        "",
        "Registry contract and usage notes:",
        "registry/README.md",
        "",
        "Registry schema:",
        manifest["entry_schema"],
        "",
        "Curated paper entries:",
        "registry/papers/",
        "",
        "Lean project root:",
        "GenLimitLean/",
        "",
        "Human mathematical maps:",
        "GenLimitLean/PaperMaps/",
        "",
        "Snapshot-specific audit evidence:",
        "GenLimitLean/AuditRecords/",
        "",
        "Important interpretation rules:",
    ]

    if identity_labels:
        claim_verb = "has" if claim_indexed_count == 1 else "have"
        identity_verb = "has" if len(identity_labels) == 1 else "have"
        lines.extend(
            [
                "- Registry v{} has one paper card for each of the {} paper umbrellas imported".format(
                    manifest["schema_version"], paper_count
                ),
                "  by GenLimit.lean. {} {} detailed claim cards, while {} {}".format(
                    claim_indexed_count,
                    claim_verb,
                    english_join(identity_labels),
                    identity_verb,
                ),
                "  identity-only entries whose source-claim and Lean-declaration inventories are",
                "  marked `not-started`.",
            ]
        )
    else:
        lines.extend(
            [
                "- Registry v{} has one claim-indexed paper card for each of the {} paper".format(
                    manifest["schema_version"], paper_count
                ),
                "  umbrellas imported by GenLimit.lean.",
            ]
        )

    lines.extend(
        [
            "- An identity card with empty claim_ids and not-started inventories establishes",
            "  paper identity only; it does not say that the paper has no claims or that its",
            "  Lean development has no declarations.",
            "- Use index.json facets `by_paper_entry_kind`,",
            "  `by_claim_inventory_completeness`, and",
            "  `by_lean_declaration_inventory_completeness`, together with their adjacent",
            "  `*_scope` facets, to distinguish indexed claims from work that has not yet",
            "  been inventoried. Completeness is meaningful only inside its declared scope.",
            "- A source-published claim with formalization coverage \"none\" is not a new",
            "  conjecture; use source_assessment to distinguish accepted, qualified,",
            "  disputed, corrected, and refuted source claims.",
            "- `formalization_frontier` is a formalization-gap view, not a conjecture list.",
            "  For component-scoped gaps, inspect each `missing_components` disposition and",
            "  reason_codes; a `not-planned` false literal reading is not a proof target.",
            "- Lean declarations linked from claim-indexed cards and their defining modules",
            "  are checked by {}.".format(manifest["generated_outputs"]["lean_audit"]),
            "- Use declarations.jsonl to find concrete definitions and helper theorems by",
            "  their actual Lean types and dependencies. Paper attribution still comes from",
            "  the claim cards and their scoped source/audit metadata.",
            "- A successful Lean build is not a source-correspondence audit; use each card's",
            "  scoped audit references.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main():
    ...
    try:
        ...
        payloads = {
            ...
            REPOSITORY_ROOT / PurePosixPath(manifest["generated_outputs"]["llms"]): render_llms(manifest, index),
        }
        if args.check:
            results = [

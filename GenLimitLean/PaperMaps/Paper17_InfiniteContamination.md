...
| Algorithm 2 / Lemma 4.3 | `finiteContamination_expansionWitness` and `lemma_4_3_*_finiteExpansion_transfer` in [`FiniteExpansionTransfer.lean`](../GenLimit/Paper17_InfiniteContamination/FiniteExpansionTransfer.lean) | Partial / specialized | The finite-expansion representation and element/set transfer are proved.  The complete Algorithm 2 presentation and the positive-density generator used later in Lemma 6.9 are not reconstructed. |
...
| Theorem 6.1 | — | Open | The positive set-generation algorithm has not been formalized. |
...
| Theorem 6.5 | `theorem_6_5_lowerDensity_necessity` and `theorem_6_5_lowerDensity_complete_necessity` in [`FiniteContaminationNecessity.lean`](../GenLimit/Paper17_InfiniteContamination/FiniteContaminationNecessity.lean) | Partial | Necessity is complete, including the alternating-prefix adversary.  Sufficiency is not formalized. |
| Lemma 6.9 | finite-difference and transfer lemmas in [`FiniteExpansionTransfer.lean`](../GenLimit/Paper17_InfiniteContamination/FiniteExpansionTransfer.lean) | Partial | The deterministic finite-expansion algebra is proved; the required noiseless positive-density construction is open. |
...
1. Formalize Theorem 6.1 and the sufficiency direction of Theorem 6.5.  This
   closes the central set-density characterization and supplies the missing
   positive construction needed by the existing finite-expansion transfer.
2. Generalize the Theorem 6.4 witness from the exact `1/2` example to an
   arbitrary admissible constant `c`.
3. Add Theorems 6.11 and 6.14 for vanishing and constant noise.
4. Add the element-density results, Theorems 6.15--6.18.
5. Reconstruct Algorithm 9 and prove Theorem 7.8 on top of the completed
   bounded-displacement infrastructure.

The order is deliberate: steps 1--4 build the missing density theory, while
step 5 can reuse the already completed Proposition 7.4 / Lemma 7.5 machinery.

# Paper 08 — Karbasi--Montasser--Sous--Velegkas paper map

Native Lean umbrella: `GenLimit.HallucinationDetection`. The cross-paper
Theorem A.2 entry point is added by
`GenLimit.Bridges.LiRamanTewariToHallucinationDetection` and the global
`GenLimit` umbrella.
...
- completed AI-assisted statement-faithfulness audit:
  ...
  checksum-pinned in
  [the audit record](../AuditRecords/HallucinationDetection/record.json).
...
appendix generation interface is connected to Li--Raman--Tewari Corollary 3.6.
...
| Corollary 2.2 | `corollary_2_2` | Complete, including empty indexed languages; generic identification-to-`ConditionTwo` necessity lives in `GenLimit.Angluin`, with a Paper 08 wrapper |
...
| Theorem A.2 | `theorem_A_2` | Complete; physically in `GenLimit.Bridges.LiRamanTewariToHallucinationDetection` because it uses LRT Corollary 3.6 |
